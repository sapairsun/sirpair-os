...
arp_resolve(struct net_iface *iface, ip_addr_t pa, uint8_t *ha)
{
    ...
    if (!cache) {
        ...
        gettimeofday(&cache->timestamp, NULL);
        /*
         * 发送 ARP 请求时不能持锁：收包路径会 net_input_handler→arp_input→mutex_lock，
         * 与同一线程上的自旋锁重入死锁，导致应答无法并入缓存，首包 IP 永远失败。
         */
        mutex_unlock(&mutex);
        arp_request(iface, pa);
        /*
         * 应答帧可能已到达网卡 RX 环，但尚未经 netif_process_rxq→deliver→arp_input；
         * 若不先收包，重入加锁时缓存仍为 INCOMPLETE，首包仍失败。
         */
        net_poll();
        net_protocol_handler();
        mutex_lock(&mutex);
        cache = arp_cache_select(pa);
        if (cache && cache->state == ARP_CACHE_STATE_RESOLVED) {
