...
#define TCP_CONNECT_TIMEOUT_TICKS 3000 /* 约 30 秒（100Hz） */
...
/*
 * 在持有 microps mutex 时执行“可中断的一拍等待”：
 * - 每拍检查当前进程是否已被 kill；
 * - 可选连接超时（ticks）；
 * - 不依赖特定 wakeup 通道，避免丢唤醒导致永久阻塞。
 */
static int
tcp_wait_tick_interruptible(uint start_ticks, uint timeout_ticks)
{
    uint now;
    ...
    acquire(&tickslock);
    now = ticks;
    if (proc_is_killed_current()) {
        release(&tickslock);
        mutex_lock(&mutex);
        errno = EINTR;
        return -1;
    }
    if (timeout_ticks > 0 && (int)(now - start_ticks) >= (int)timeout_ticks) {
        release(&tickslock);
        mutex_lock(&mutex);
        errno = ETIMEDOUT;
        return -1;
...
tcp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
    ...
    if (cksum16((uint16_t *)hdr, len, psum) != 0) {
        if (tcp_input_dbg < 64) {
            cprintf("tcp_input dbg: checksum drop sum=0x%x\n", ntoh16(hdr->sum));
            tcp_input_dbg++;
        }
    ...
    if (hlen < sizeof(*hdr) || hlen > len) {
        /* 某些报文头长字段异常，回退为最小 TCP 头，避免将有效负载误判为无效。 */
        hlen = sizeof(*hdr);
    }
...
AGAIN:
    ...
    while (pcb->state == state) {
        if (tcp_wait_tick_interruptible(wait_start, TCP_CONNECT_TIMEOUT_TICKS) == -1) {
            debugf("interrupted");
            pcb->state = TCP_PCB_STATE_CLOSED;
            tcp_pcb_release(pcb);
            mutex_unlock(&mutex);
...
tcp_send(int id, uint8_t *data, size_t len)
{
    ...
    struct ip_iface *iface;
    size_t mss, cap, slen;
    static int send_dbg_count;
    uint wait_start = 0;

    if (send_dbg_count < 128) {
        cprintf("tcp_send dbg: enter id=%d len=%u\n", id, len);
        send_dbg_count++;
    }
    mutex_lock(&mutex);
    if (send_dbg_count < 128) {
        cprintf("tcp_send dbg: lock acquired id=%d\n", id);
        send_dbg_count++;
    }
    pcb = tcp_pcb_get(id);
    if (!pcb) {
...
RETRY:
    if (send_dbg_count < 128) {
        cprintf("tcp_send dbg: state id=%d state=%d snd_wnd=%u nxt=%u una=%u\n",
                id, pcb->state, pcb->snd.wnd, pcb->snd.nxt, pcb->snd.una);
        send_dbg_count++;
    }
    switch (pcb->state) {
    case TCP_PCB_STATE_CLOSED:
    ...
    case TCP_PCB_STATE_CLOSE_WAIT:
        ...
        while (sent < (ssize_t)len) {
            ...
            if (!cap) {
                /*
                 * 某些对端在三次握手后窗口更新到达较慢，可能导致首包发送前
                 * snd.wnd 仍为 0，用户态表现为 connect 成功后 send 长时间阻塞。
                 * 若当前无未确认数据，允许先探测发送一个 MSS 片段，后续再由 ACK
                 * 正常收敛窗口。
                 */
                if (pcb->snd.wnd == 0 && pcb->snd.nxt == pcb->snd.una)
                    cap = mss;
            }
            if (!cap) {
                if (send_dbg_count < 128) {
                    cprintf("tcp_send dbg: wait cap=0 id=%d state=%d wnd=%u nxt=%u una=%u\n",
                            id, pcb->state, pcb->snd.wnd, pcb->snd.nxt, pcb->snd.una);
                    send_dbg_count++;
                }
                if (tcp_wait_tick_interruptible(wait_start, 0) == -1) {
                    debugf("interrupted");
                    if (!sent) {
                        mutex_unlock(&mutex);
                        errno = EINTR;
            ...
            if (send_dbg_count < 128) {
                cprintf("tcp_send dbg: output id=%d slen=%u cap=%u wnd=%u nxt=%u una=%u\n",
                        id, slen, cap, pcb->snd.wnd, pcb->snd.nxt, pcb->snd.una);
                send_dbg_count++;
            }
...
int
tcp_sock_listen_readable(int id)
{
    struct tcp_pcb *pcb;
    int r;
