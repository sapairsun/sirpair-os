...
void
ip_route_remove_for_iface(struct ip_iface *iface)
{
  struct ip_route **pp;
  struct ip_route *r;
...
int
ip_iface_reconfigure(struct ip_iface *iface, const char *unicast, const char *netmask,
                     const char *gateway)
{
  ip_route_remove_for_iface(iface);
  ...
  if (!ip_route_add(iface->unicast & iface->netmask, iface->netmask, IP_ADDR_ANY, iface)) {
    errorf("ip_route_add() failure (connected)");
    return -1;
  }
  return ip_route_set_default_gateway(iface, gateway);
}
...
static int
ip_output_device(struct ip_iface *iface, const uint8_t *data, size_t len, ip_addr_t nexthop_l2)
{
    uint8_t hwaddr[NET_DEVICE_ADDR_LEN] = {};
    int ret;

    if (NET_IFACE(iface)->dev->flags & NET_DEVICE_FLAG_NEED_ARP) {
        if (nexthop_l2 == iface->broadcast || nexthop_l2 == IP_ADDR_BROADCAST) {
            memcpy(hwaddr, NET_IFACE(iface)->dev->broadcast, NET_IFACE(iface)->dev->alen);
        } else {
            ret = arp_resolve(NET_IFACE(iface), nexthop_l2, hwaddr);
            /*
             * ARP_RESOLVE_INCOMPLETE 为 0，与 net_device_output 成功时的返回值相同；
             * 若原样返回 ret，ip_output 会误判为已发送。未完成解析时必须返回 -1。
             */
...
static mutex_t ip_id_mutex;
static uint16_t ip_id_counter = 128;
...
ip_generate_id(void)
{
    ...
    mutex_lock(&ip_id_mutex);
    ret = ip_id_counter++;
    mutex_unlock(&ip_id_mutex);
    return ret;
}
