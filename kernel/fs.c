...
//   + Names: paths like /usr/rtm/sirpair/fs.c for convenient naming.
//
...
// listed in block ip->addrs[NDIRECT]. Remaining blocks are
// via a double-indirect block at ip->addrs[NDIRECT+1].
...
// Return mapped disk block for reading without allocating new blocks.
// Returns 0 when the logical block is a hole or not mapped.
static uint
...
// 填充根设备上的文件系统统计（数据块空闲位图 + inode 空闲计数）
void
fillstatfs(int dev, struct statfs *st)
{
  struct superblock sb;
  struct buf *bp;
  struct dinode *dip;
  uint b, datastart, freeb, freei, inum;
  int bi, m;

  readsb(dev, &sb);
  st->f_bsize = BSIZE;
  st->f_blocks = sb.nblocks;
  datastart = sb.size - sb.nlog - sb.nblocks;

  freeb = 0;
  for(b = datastart; b < datastart + sb.nblocks; b++){
    bp = bread(dev, BBLOCK(b, sb.ninodes));
    bi = b % BPB;
    m = 1 << (bi % 8);
    if((bp->data[bi/8] & m) == 0)
