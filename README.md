...
<blockquote>
想要获取最新进展，请访问 Sirpair OS 网址主页：<a href="http://sirpair.com">http://sirpair.com</a>
</blockquote>

Sirpair OS 是一个基于 xv6 深度扩展的 32 位 x86 操作系统，目标硬件为 ThinkPad X220，同时支持在 QEMU 中运行。项目在保留 xv6 教学型内核结构的基础上，重点增强了 USB 启动、网络、图形、用户态工具链和第三方程序移植能力。
...
Sirpair OS 是一个基于 xv6 深度扩展的 32 位 x86 操作系统，目标硬件为 ThinkPad X220，同时支持在 QEMU 中运行。项目在保留 xv6 教学型内核结构的基础上，重点增强了以下方向：
...
与传统 xv6 常见的 IDE 路径不同，Sirpair OS 明确转向：
...
Sirpair OS 延续了 xv6 的简洁文件系统模型，并保留了日志机制用于元数据一致性保护。系统支持：
...
Sirpair OS 保留了 xv6 清晰直接的结构，同时围绕真实硬件能力做了更多增强，尤其是：
...
Sirpair OS 保留了 xv6 的 SMP 设计思路，但做了更保守的同步控制。
...
系统使用 xv6 风格的物理页分配器 `kalloc`，但内核启动时采用两阶段初始化：
...
系统使用页表管理内核和用户地址空间，并保留了 xv6 的典型模型：
...
这使系统在交互式 shell 场景下比原始 xv6 更实用。

### 6. 文件系统与日志架构

#### 6.1 文件系统组织

文件系统沿用 xv6 的 inode、目录项和块缓存模型，主要模块包括：
...
Sirpair OS 的系统调用接口明显比原始 xv6 更丰富，主要分为以下几类：
...
Sirpair OS 的设计不是单纯把 xv6 补几个命令，而是围绕“可在模拟器和真机上运行的轻量实验操作系统”这一目标做了系统性扩展。
