...
#ifdef SIRPAIR
static volatile int drain_mode = 0;
#else
...
fmt_stats(char *buf, size_t size, void *x)
{
    ...
    Server *s = x;
#ifdef SIRPAIR
    int ut_sec = 0, ut_usec = 0, st_sec = 0, st_usec = 0;
#else
    struct rusage ru;
#endif
...
conn_process_io(Conn *c)
{
    ...
    case STATE_SEND_JOB:
        ...
        if (c->reply_sent < c->reply_len) {
            ...
            r = write(c->sock.fd, j->body + c->out_job_sent, j->r.body_size - c->out_job_sent);
        }
        if (r == -1) {
            check_err(c, "write()");
            return;
...
prot_init()
{
...
#ifdef SIRPAIR
    {
        ...
        x = (uint32)getpid() ^ (uint32)uptime() ^ (uint32)nanoseconds();
        for (i = 0; i < instance_id_bytes; i++) {
            x = x * 1103515245U + 12345U;
            rand_data[i] = (byte)(x >> 16);
        }
        for (i = 0; i < instance_id_bytes; i++) {
            sprintf(instance_hex + (i * 2), "%02x", rand_data[i]);
        }
        memcpy(node_info.nodename, "spa\0", 4);
        memcpy(node_info.version, "spa\0", 4);
        memcpy(node_info.machine, "i386\0", 5);
    }
#else
