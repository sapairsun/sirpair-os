...
#ifdef SIRPAIR_LUA
  /*
   * Sirpair 用户程序以 -e main 链接，栈上伪造返回地址为 0xffffffff。
   * 若从 main 直接 return，会在用户态跳转到 0xffffffff 触发缺页，内核在
   * 异常路径中调用 exit()，与 SMP/锁状态组合时曾偶发 panic("acquire")。
   * 必须显式调用 exit()，经系统调用正常退出。
   */
  exit((result && status == LUA_OK) ? 0 : 1);
#else
