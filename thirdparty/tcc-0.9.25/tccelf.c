...
#else
        /* 映像内库位于 /tcc/lib/（见 tools/mkfs.c），与 CONFIG_TCCDIR=/tcc 组合成完整路径。
         * Sirpair 上使用 libtcc1_rt.o（ET_REL）而非 libtcc1.a：归档解析在部分环境下易与裸机 I/O
         * 交互出问题；单对象路径与 tcc_load_object_file 一致，保证 __udivdi3 等符号被合并。 */
#ifdef SIRPAIR_TCC
        snprintf(buf, sizeof(buf), "%s/lib/%s", s1->tcc_lib_path, "libtcc1_rt.o");
#else
...
#endif
#ifdef SIRPAIR_TCC
        snprintf(buf, sizeof(buf), "%s/lib/%s", s1->tcc_lib_path, "libsirpairrt.a");
        tcc_add_file(s1, buf);
#endif
    }
    ...
    if (phnum > 0) {
        ...
        if (s1->has_text_addr) { 
            ...
            if (file_type == TCC_OUTPUT_DLL)
                ...
                addr = ELF_START_ADDR;
#ifndef SIRPAIR_TCC
            /* compute address after headers（与文件偏移同余于页，供 Linux 加载器使用） */
            addr += (file_offset & (ELF_PAGE_SIZE - 1));
#else
            /* Sirpair kernel/exec.c 的 loaduvm 要求 ph.vaddr 按页对齐；不能用 vaddr≡off(mod 页) 的非对齐 vaddr。
             * 将首段起始文件偏移对齐到页边界，并保持 p_vaddr=ELF_START_ADDR（已对齐）。 */
            if (file_type == TCC_OUTPUT_EXE)
...
#else
                /* Sirpair：第二段 PT_LOAD 的 p_vaddr 也必须页对齐；原 ELF 分支仅 addr+=4096 无法对齐到页边界 */
                addr = (addr + ELF_PAGE_SIZE - 1) & ~(ELF_PAGE_SIZE - 1);
                file_offset = (file_offset + ELF_PAGE_SIZE - 1) & ~(ELF_PAGE_SIZE - 1);
#endif
            }
...
#error unsupported CPU
    ...
    if (file_type != TCC_OUTPUT_OBJ) {
        ...
        if (file_type == TCC_OUTPUT_EXE) {
            ...
            {
                void *ep;
                ep = tcc_get_symbol(s1, "_start");
                if(ep)
                    ehdr.e_entry = (unsigned long)ep;
                else
                    ehdr.e_entry = (unsigned long)tcc_get_symbol_err(s1, "main");
            }
#else
